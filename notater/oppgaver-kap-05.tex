...
\begin{center}
	\begin{tikzpicture}[scale=0.6,baseline=(O)]
...
\hspace{10pt}
	\begin{tikzpicture}[scale=0.6,baseline=(O)]
	\draw[->,above] (0,0) node{} -- (1,2) node{$\V{w}_1$};
	\draw[->, right] (0,0) node{} -- (2,1) node{$\V{w}_2$};
	\draw[->, right] (0,0) node{} -- (2,-1) node{$\V{w}_3$};
	\draw[->] (-1,0) {} -- (3,0) {};
	\draw[->] (0,-1) {} -- (0,3) {};
        \node (O) at (0,0) {};
	\end{tikzpicture}
\end{center}
I hvert tilfelle:
Er vektorene på tegningen lineært uavhengige?
Utspenner de~$\R^2$?
Begrunn svarene dine.
\end{oppgave}

\begin{losning}
Vektorene $\V{v}_1$ og~$\V{v}_2$ er lineært uavhengige og spenner
ut~$\R^2$.

Vektorene $\V{w}_1$, $\V{w}_2$ og~$\V{w}_3$ er lineært avhengige og
spenner ut~$\R^2$.
\end{losning}
...
\begin{punkt}
% TODO omformuler (eller fjern) denne oppgaven
